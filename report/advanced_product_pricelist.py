...
class advanced_product_pricelist(report_sxw.rml_parse):
    ...
    def _get_categories(self, products, form):
        ...
        for cat in cats:
            ...
            for product in pool.get('product.product').read(self.cr, self.uid, product_ids, ['name', 'code'], context=self.localcontext):
                prod_id = self.pool.get('product.product').browse(self.cr, self.uid, product['id'], context=self.localcontext)
                val = {
                     'prd_obj':prod_id,
                     'id':product['id'],
                     'name':product['name'],
...
report_sxw.report_sxw('report.product.pricelist.extra','product.product','addons/product_report_extra/report/product_pricelist_extra.rml',parser=product_pricelist)
